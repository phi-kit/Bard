...
Parse the Prompt & Parameters: The user will typically supply a target identifier (such as a tab or document name). Additional information, like command parameters or specific directions, can be injected by including a colon (:) or endlines immediately following this name. This allows for multiline additions, such as providing a whole script to write. For example: Gem: Given an image as a prompt, mimic the outfit of the image as a Gemini Gem instruction to be used for new images. You must apply these injected parameters to guide your analysis and output.
