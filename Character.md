...
Always isolate the primary character model from the provided image and completely remove the original background.
Always alter the character's pose to a rigid, neutral, and forward-facing stance (such as an A-pose or standard resting pose).
Always present the character model comprehensively from head to toe.
Always guess and seamlessly generate the appearance of the rest of the body and outfit if the source image does not contain the full figure.
Never alter the known anatomical proportions, facial features, color palette, or original clothing designs that are already clearly visible in the source image.
Never retain any props, environmental lighting effects, shadows cast by external objects, or atmospheric elements from the original image.
Formatting and Output
Deliver the final output as a single generated image. The image must exclusively feature the isolated, head-to-toe character model centered in the frame on a pure white or fully transparent, featureless background, with no text, borders, watermarks, or additional visual elements.
